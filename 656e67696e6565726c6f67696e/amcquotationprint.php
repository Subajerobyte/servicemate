...
$quoteid=mysqli_real_escape_string($connection, $_GET['id']);
		$sqlselect = "SELECT * From jrcamcquotation where engineerid='".$id."' and id='".$quoteid."' group by qno, qdate order by id desc";
...
$addaterms="";
$quotationtypes="";
$sqlselect3=mysqli_query($connection, "select quotationtype from jrcamcquotation where qno='".$rowselect['qno']."' and qdate='".$rowselect['qdate']."' and quotationtype='AMC' or quotationtype='CAMC'");
$countselect3=mysqli_num_rows($sqlselect3);
$ci=1;
while($rowselect3=mysqli_fetch_array($sqlselect3))
{
$sqlquotation=mysqli_query($connection, "select quotationtype, terms from   where id='".$rowselect3['quotationtype']."'");
while($infoamcquotation=mysqli_fetch_array($sqlquotation))
{
	if($infoamcquotation['terms']!="")
	{
	$addterms.="<p style='font-style:italic'>".$infoamcquotation['quotationtype'].":</p>";
	$addterms.=$infoamcquotation['terms'];
	}
...
<body onLoad="window.print()">
  ...
  <table>
      ...
      <tr>
        <td>
		  ...
		  <?=$rowcons['phone']?> <?=$rowcons['mobile']?>
		  		  ...
		  		  <?php
		 $sqlselectx = "SELECT * From jrcamcquotation where id='".$quoteid."' ";
        $queryselectx = mysqli_query($connection, $sqlselectx);
        $rowCountselect = mysqli_num_rows($queryselectx);
		$rowselect = mysqli_fetch_array($queryselectx);
        if(!$queryselectx){
           die("SQL query failed: " . mysqli_error($connection));
        }
		...
		?>
		  ...
		  <strong>Sub: <?=$infoamcquotation['subject']?> <?=$quotationtypes?></strong> <br>
		  Pleasant Greetings,
		  <br>
	...
	<tr>
	<td><?=$i?></td>
	<td><b><?=$rowxl['stockitem']?></b><br>Serial No:<?=nl2br($rowselect['serialnumber'])?></td>
