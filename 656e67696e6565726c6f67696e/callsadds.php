...
mysqli_query($connection, "UPDATE jrcsearchhistory set  calltid='".$calltid."' where consigneeid='".$consigneeidvalue."' and engineerid='".$engineerid."'");
...
$rowxl['email']=jbsdecrypt($_SESSION['encpass'], $rowxl['email']);
		...
		$sqleng1 = "SELECT id, callhandlingid, coordinatorid From jrccalls where calltid='$calltid'";
		$queryeng1 = mysqli_query($connection, $sqleng1);
        $rowCounteng1 = mysqli_num_rows($queryeng1);
        if(!$queryeng1){
           die("SQL query failed: " . mysqli_error($connection));
        }
		$roweng1 = mysqli_fetch_array($queryeng1);
		$sqleng = "SELECT username From jrcadminuser where (id='1' or id='".$roweng1['callhandlingid']."' or id='".$roweng1['coordinatorid']."') order by username asc";
		$queryeng = mysqli_query($connection, $sqleng);
        $rowCounteng = mysqli_num_rows($queryeng);
        if(!$queryeng){
           die("SQL query failed: " . mysqli_error($connection));
...
if($servicetype=='Carry-In')
{
header("Location: calls.php?remarks=Added Successfully");
}
else
{
header("Location: calls.php?id=".$consigneeidvalue."&remarks=Added Successfully");
}
}
elseif($i==0 && $j!=0)
{
header("Location: calls.php?id=".$consigneeidvalue."&error=All Calls Already Inserted A Complaint is Open for this Customer and Product! Kindly Close it");
}
elseif($i==0 && $j==0)
{
header("Location: calls.php?id=".$consigneeidvalue."&error=NO Calls Inserted");
}
else
{
header("Location: calls.php?id=".$consigneeidvalue."&remarks=Already A Complaint is Open for this Customer and Product! Kindly Close it");
}
}
}
else
