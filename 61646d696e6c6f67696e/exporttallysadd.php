...
<title><?=$_SESSION['companyname']?> - Jerobyte - Add New Service Sales Order</title>
...
<h1 class="h4 mb-2 mt-2 text-gray-800">Add New Service Sales Order</h1>
</div>
<?php
...
<div class="col-lg-4">
<div class="form-group">
<label for="invoicenoto">Invoice Number (Upto)</label>
...
<td><input type="number" stag="<?=$i?>" onchange="changeconsigneeinfo(this.getAttribute('stag'), this.value,  'conmobile')"  class="conmobile" name="conmobile[]" id="conmobile<?=$i?>" placeholder="MOBILE NO. (<?=$i?>)" tabindex="30"></td>
...
<td><input type="text"  class="conprogroup" name="conprogroup[]" id="conprogroup<?=$i?><?=$j?>" placeholder="PRODUCT GROUP (<?=$i?>)" tabindex="33"></td>
<td><input type="text"  class="conmultiple" name="conmultiple[]" id="conmultiple<?=$i?><?=$j?>" placeholder="MULTIPLE GODOWN (<?=$i?>)" tabindex="34"></td>
...
<td><input type="number" step="0.01" min="0" class="conunit" name="conunit[]" id="conunit<?=$i?><?=$j?>" placeholder="UNIT PRICE  (<?=$i?>)" onchange="procalc(<?=$i?>)" tabindex="39"></td>
...
$( "#conproductcode<?=$i?><?=$j?>" ).autocomplete({
//source: 'pincodesearch.php?type=pincode',
source: 'productsearch1.php?type=code', select: function (event, ui) { $("#conproductcode<?=$i?><?=$j?>").val(ui.item.value); $("#conproduct<?=$i?><?=$j?>").val(ui.item.stockitem); $("#conmarketname<?=$i?><?=$j?>").val(ui.item.marketname);$("#conunit<?=$i?><?=$j?>").val(ui.item.price);}, minLength: 2
});
<?php
}
}
