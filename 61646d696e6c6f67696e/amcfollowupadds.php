...
if(isset($_POST['submit']))
{
		...
		$followuptype="AMC Quotation Followup";
	if($followupdate!="")
	{		
		...
		{	
			...
			else
			{
				...
				mysqli_query($connection, "INSERT INTO jrchistory (user, ipaddress, times, remarks, tid, tablename) VALUES ('{$useremail}', '{$ip}', '{$times}', 'Insert A New AMC Followup', '{$tid}', 'jrcfollowup')");
				if($status=='Postponed')
				{
				 mysqli_query($connection, "INSERT INTO jrcreminder (createdon, remindertype, sourceid, reminder, enddate) VALUES ('{$createdon}', '{$followuptype}', '{$referqno}','AMC Quotation Followup Reminder  for Quotation Number {$referqno}', '{$followupback}')");
				}
				header("Location: amcquotationgenview.php?id='".$referqno."'&remarks=Added Successfully");
			} 
	    }
		else
			{
				header("Location: amcquotationgenview.php?id='".$referqno."'&error=This record is Already Found! Kindly check in All Administrator List");
			}
	}
	else
			{
				header("Location: amcquotationgenview.php?id='".$referqno."'&error=Error Data");
			}
}
else
			{
				header("Location: amcquotationgenview.php?id='".$referqno."'");
			}
