...
<title>AMC Customers | Jerobyte - Going to Expire AMC Customers</title>
...
<div class="row">
<div class="col">
<h1 class="h4 mb-2 mt-2 text-black-800 text-center"><b>Going to Expire AMC Customers</b></h1>
</div>
</div>
<?php
...
<label for="noofdays">AMC To Expire (No of Days)</label>
<input type="number" min="0" class="form-control" id="noofdays" name="noofdays" placeholder="Enter the Number of Days for AMC Expiry" value="<?=(isset($_POST['noofdays']))?$_POST['noofdays']:((isset($_GET['day']))?'90':'')?>" >
</div>
</div>
<div class="col-lg-3 text-left">
<div class="form-group">
...
$sqlamc1 = "SELECT sourceid,dateto From jrcamc  order by id asc";
...
if($rowamc1['dateto']!='')
{
$date = new DateTime($rowamc1['dateto']);
$now = new DateTime();
if($date >= $now) {
$_POST['noofdays']='';
$datediff = strtotime($rowamc1['dateto']) - time();
$days=round($datediff / (60 * 60 * 24));
if(($days>0)&&(($days<(int)$_POST['noofdays'])|| ($days<(int)$_GET['day'])) )
{
$sqlselect = "SELECT maincategory, subcategory, department, consigneename, address1, address2, area, district, pincode, contact, phone, mobile, email, invoiceno, installedby, stockmaincategory, stocksubcategory, stockitem, overallwarranty, typeofproduct, componenttype, componentname, make, capacity, installedon, invoicedate, warranty, id, qty, consigneeid, serialnumber, departments From jrcxl  where id='".$rowamc1['sourceid']."' ".$staqu." ";
$queryselect = mysqli_query($connection, $sqlselect);
$rowCountselect = mysqli_num_rows($queryselect);
if(!$queryselect){
die("SQL query failed: " . mysqli_error($connection));
...
?><br><a href="callsadd.php?id=<?=$rowselect['id']?>&t=am" class="btn btn-danger btn-sm">Take A AMC Call</a><br>
...
$sqlamc1 = "SELECT sourceid,dateto From jrcamc where (dateto between '$today' and '$next90day')   order by id asc";
$queryamc1 = mysqli_query($connection, $sqlamc1);
$rowCountamc1 = mysqli_num_rows($queryamc1);
if(!$queryamc1){
die("SQL query failed: " . mysqli_error($connection));
