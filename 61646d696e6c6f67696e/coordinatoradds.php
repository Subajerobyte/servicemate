...
if(isset($_POST['submit']))
{
  ...
  if (isset($_POST['submit'])) {
	...
	{		
        ...
        $sqlcon = "SELECT id From jrccoordinator WHERE username = '{$usernamevalue}'";
		...
		{	
			// Query if email exists in db
			$sqlup = "INSERT INTO jrccoordinator( coordinatorname, designation, username, password, address1, address2, area, district, pincode, contact, phone, mobile, email, avatar) VALUES ( '$coordinatornamevalue', '$designationvalue', '$usernamevalue', '$passwordvalue', '$address1value', '$address2value', '$areavalue', '$districtvalue', '$pincodevalue', '$contactvalue', '$phonevalue', '$mobilevalue', '$emailvalue', '$avatar')";
			...
			else
			{
				...
				mysqli_query($connection, "INSERT INTO jrchistory (user, ipaddress, times, remarks, tid) VALUES ('{$useremail}', '{$ip}', '{$times}', 'Insert A Call Co-ordinator', '{$tid}')");
				header("Location: coordinator.php?remarks=Added Successfully");
			} 
	    }
		else
			{
				header("Location: coordinator.php?error=This record is Already Found! Kindly check in All Call Co-ordinator List");
			}
	}
	else
			{
