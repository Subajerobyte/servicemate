...
<head>
  ...
  <title><?=$_SESSION['companyname']?> - Jerobyte - AMC Expiry Alert</title>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
		  ...
		  <div class="row">
  <div class="col">
    <h1 class="h4 mb-2 mt-2 text-black-800 text-center"><b>AMC Expiry Alert</b></h1>
  </div>
</div>
...
$soure="";
		$sqlsels = "SELECT sourceid From jrcreminder where remindertype='AMC EXPIRE' and enabled='0' order by sourceid asc";
                ...
                <table class="table table-bordered font-13" id="dataTable" width="100%" cellspacing="0">
                  <thead>
                    <tr>
					  ...
					  <th>Edit</th>
					  <th>Take Call</th> 
					  <th>Generate AMC Quotation</th>
					  <th>Pending/Completed</th>
					 </tr>
                  </thead>
                  <tbody>
				<?php
				$sqlselect = "SELECT sourceid, createdon, reminder, enddate, status, id, enabled From jrcreminder where remindertype='AMC EXPIRE' and enabled='0' ".$sour." ".$staqu1." order by DATE(enddate) asc";
				$queryselect = mysqli_query($connection, $sqlselect);
				$rowCountselect = mysqli_num_rows($queryselect);
				if(!$queryselect){
				   die("SQL query failed: " . mysqli_error($connection));
		...
		$rowcons['email']=jbsdecrypt($_SESSION['encpass'], $rowcons['email']);
					  ...
					  <td><?=$rowxl['stocksubcategory']?> - <span class="text-primary"><?=$rowxl['stockitem']?></span> - Rs.<?=$totprice?></td>
							  ...
							  if($rowselect['enabled']=='1')
							  {
								...
								<td></td>
								<td>
								<a href="reminderchange.php?id=<?=$rowselect['id']?>&val=0" onclick="return confirm('Are you sure want to make Pending this AMC Expire Alert?')" ><span class="text-success">Completed</span></a>
								</td>
								<?php 
							  }
							  else
							  {
								  ?>
								  <td><a target="_blank" href="callsadd.php?id=<?=$rowxl['id']?>&ts=amcexpire&ts1=<?=$rowselect['id']?>" class="btn btn-primary btn-sm">Take Call</a></td>
								  <td><a target="_blank" href="amcquotationgen.php?id=<?=$rowcons['id']?>&xlid=<?=$rowxl['id']?>&ts=amcexpire&ts1=<?=$rowselect['id']?>" class="btn btn-primary btn-sm">Generate AMC Quotation</a></td>
								<td><a href="reminderchange.php?id=<?=$rowselect['id']?>&val=1" onclick="return confirm('Are you sure want to Complete this AMC Expire Alert?')" ><span class="text-danger">Pending</span></a>
								</td>
