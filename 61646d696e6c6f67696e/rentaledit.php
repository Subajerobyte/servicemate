...
<title><?=$_SESSION['companyname']?> - Jerobyte - Edit Rental Details</title>
...
<h1 class="h4 mb-2 mt-2 text-gray-800">Edit Rental Details</h1>
</div>
<?php
...
$rono=mysqli_real_escape_string($connection,$_GET['id']);
				  $sqlselect = "SELECT * From jrcrental where rono='".$rono."' order by id asc";
...
<div class="card-body">
<form method="post" method="post" action="rentaledits.php" enctype="multipart/form-data">
<input type="hidden" name="id" id="id" value=<?=$rowselect[0]['id']?>>
<input type="hidden" name="oconsigneeid" id="oconsigneeid" value=<?=$rowselect[0]['consigneeid']?>>
...
<div class="col-md-12">
<div class="mt-1 text-primary mb-1">Consignee Details <input type="hidden" name="consigneeid" id="consigneeid" value="<?=$rowselect[0]['consigneeid']?>"> <span href="#" data-toggle="modal" data-target="#consigneemodal" onclick="updateauto()">&nbsp;<i class="fa fa-pen"></i> </span></div>
...
<div class="col-lg-6">
<div class="form-group">
<label for="rono">RO No</label>
<input type="text" class="form-control" id="rono" name="rono" placeholder="RO No" value="<?=$rowselect[0]['rono']?>" readonly>
</div>
</div>
<div class="col-lg-6">
<div class="form-group">
<label for="shipment">Rental Date</label>
<input type="date" class="form-control" id="rentaldate" name="rentaldate" value="<?=$rowselect[0]['rentaldate']?>">
</div>
</div>
<div class="col-lg-6">
<div class="form-group">
...
<th>PRODUCT </th><th>DURATION</th><th>QTY</th><th>RATE</th><th>PENALTY</th><th>DISCOUNT</th><th>TAX</th><th>AMOUNT</th><th></th>
</tr>
<?php
$i=1;
...
<option value="">Select</option>
<?php
$sqlgo = "SELECT id, stockitem From jrcproduct order by stockitem asc limit 200";
$querygo = mysqli_query($connection, $sqlgo);
$rowCountgo = mysqli_num_rows($querygo);
if(!$querygo){
die("SQL query failed: " . mysqli_error($connection));
...
<input type="hidden" name="id[]" id="id<?=$i?>" value="<?=$row['id']?>">
	...
	<td style="width:8%"><input type="datetime-local" class="form-control" id="datefrom<?=$i?>" name="datefrom[]" style="text-align:left" value="<?=$row['datefrom']?>" required><span href="#" class="text-danger text-right">From Date</span></br>
<input type="datetime-local" class="form-control" id="dateto<?=$i?>" name="dateto[]" style="text-align:left" value="<?=$row['dateto']?>" required><span href="#" class="text-danger text-right">To Date</span></td>
...
<td style="width:10%"><input type="number" name="penaltyperday[]" id="penaltyperday<?=$i?>" class="form-control" min="0" step="0.01" onchange="productcalc(<?=$i?>)" style="text-align:right" value="<?=$row['penaltyperday']?>">
...
<input type="hidden" name="rentper[]" id="rentper<?=$i?>" value="<?=$row['rentper']?>">

<input type="hidden" name="rentperamount[]" id="rentperamount<?=$i?>" value="<?=$row['rentperamount']?>">



<input type="hidden" name="gstamount[]" id="gstamount<?=$i?>" value="<?=$row['gstamount']?>">
</td>
<td style="width:10%"><input type="text" name="totalamount[]" id="totalamount<?=$i?>" readonly class="form-control" style="text-align:right" value="<?=$row['totalamount']?>"><div id="totalamountresponse<?=$i?>"></div>
</td>
...
<input type="hidden"  id="oldbeforeimage" name="oldbeforeimage" value="<?=$rowselect[0]['beforeimage']?>">
<input type="file" class="form-control" id="beforeimage" name="beforeimage[]" style="padding:10px; height:auto" multiple>
<label for="terms">You can upload a maximum of two files, Each with a size limit of 2MB</label>

<?php 
									if($rowselect[0]['beforeimage']!="")
									{
										
										$photos1=explode("|",$rowselect[0]['beforeimage']);
										$ext = pathinfo($row['beforeimage'], PATHINFO_EXTENSION);
										foreach($photos1 as $photo1)
										{
											if ($ext=="pdf") {
										?>
										
										<a href="#" class="pop"><embed src="<?=$photo1?>" width="200px" height="200px"  type="application/pdf"/></a>
										<?php
											}
										...
										?>
										<a href="#" class="pop"><img src="<?=$photo1?>" width="200px" height="200px" ></a>
										<?php
										}
										else{
										?>
										<a href="<?=$photo1?>" >View File</a>
										<?php
									}
										}
									}
...
<input type="hidden"  id="oldafterimage" name="oldafterimage" value="<?=$rowselect[0]['afterimage']?>">
<input type="file" class="form-control" id="afterimage" name="afterimage[]" style="padding:10px; height:auto" multiple>
<label for="terms">You can upload a maximum of two files, Each with a size limit of 2MB</label>
<?php 
									if($rowselect[0]['afterimage']!="")
									{
										
										$photos=explode("|",$rowselect[0]['afterimage']);
										$ext = pathinfo($row['afterimage'], PATHINFO_EXTENSION);
										foreach($photos as $photo)
										{
											if ($ext=="pdf") {
										?>
										
										<a href="#" class="pop"><embed src="<?=$photo?>" width="200px" height="200px"  type="application/pdf"/></a>
										<?php
											}
											else if ($ext=="img" || $ext=="png" || $ext=="jpg" || $ext=="jpeg" || $ext=="gif" ) 
											{
										...
										?>
										<a href="#" class="pop"><img src="<?=$photo?>" width="200px" height="200px" ></a>
										<?php
										}
										else{
										?>
										<a href="<?=$photo?>" >View File</a>
										<?php
									}
										}
									}
...
<textarea class="form-control" id="terms" name="terms" style="height: 90px;"><?=$rowselect[0]['terms']?></textarea>
...
<div class="col-lg-4">
<input type="hidden"  id="paymentmode" name="paymentmode" readonly value="<?=$rowselect[0]['paymentmode']?>">
<input type="hidden"  id="advanceamount" name="advanceamount" readonly value="<?=$rowselect[0]['advanceamount']?>">
...
<?php 
/* <!--div class="form-group row">
    <label for="paymentmode" class="col-sm-4 col-form-label text-right">Payment Mode </label>
    <div class="col-sm-8">
		<select name="paymentmode" id="paymentmode" class="form-control" maxlength="50" style="text-align:left">
			<option value="" <?=($rowselect[0]['paymentmode']=='')?'selected':''?>>SELECT</option>
			<option value="CASH"<?=($rowselect[0]['paymentmode']=='CASH')?'selected':''?>>CASH</option>
			<option value="CHEQUE" <?=($rowselect[0]['paymentmode']=='CHEQUE')?'selected':''?>>CHEQUE</option>
			<option value="G-PAY" <?=($rowselect[0]['paymentmode']=='G-PAY')?'selected':''?>>G-PAY</option>
			<option value="PAYTM" <?=($rowselect[0]['paymentmode']=='PAYTM')?'selected':''?>>PAYTM</option>
			<option value="PHONEPE" <?=($rowselect[0]['paymentmode']=='PHONEPE')?'selected':''?>>PHONEPE</option>
			<option value="NEFT" <?=($rowselect[0]['paymentmode']=='NEFT')?'selected':''?>>NEFT</option>
			<option value="RTGS" <?=($rowselect[0]['paymentmode']=='RTGS')?'selected':''?>>RTGS</option>
			<option value="IMPS" <?=($rowselect[0]['paymentmode']=='IMPS')?'selected':''?>>IMPS</option>
			<option value="UPI"<?=($rowselect[0]['paymentmode']=='UPI')?'selected':''?>>UPI</option>
		</select>
    </div>
  </div>
  <div class="form-group row">
    <label for="advanceamount" class="col-sm-4 col-form-label text-right font-weight-bold">Advance Amount (₹)</label>
    <div class="col-sm-8">
      <input type="text"  class="form-control" id="advanceamount" name="advanceamount" style="text-align:right" value="<?=$rowselect[0]['advanceamount']?>">
    </div>
  </div--> */
?>
...
{
?>
<option value="<?=$rowgo['id']?>"<?=($rowselect[0]['consigneeid']==$rowgo['id'])?'selected':''?>><?=$rowgo['consigneename']?></option>
<?php
}
}
?>
...
{
?>
<input type="hidden" name="chconsigneename" id="chconsigneename" value="<?=$rowselect[0]['consigneeid']?>">
<?php
}
?>
</div>
...
$clone.find('#datefrom'+oldcurid).attr("id", "datefrom"+curid);
$clone.find('#dateto'+oldcurid).attr("id", "dateto"+curid);
$clone.find('#penaltyperday'+oldcurid).attr("id", "penaltyperday"+curid);
...
var rentalminvalue= response[i]['rentalminvalue'];
...
productresponse1+='<br><label><input type="radio" name="godownname'+ids+'" id="godownname'+ids+'s'+godownid+'" value="'+godownid+'" required> '+godownname+' - '+available+'</label>';
}
...
$("#rentalminvalue"+ids).val(rentalminvalue);
$("#gst"+ids).val(gst);
$("#gstper"+ids).val(gst);
...
$("#prrate").val($("#rate"+ids).val());
$("#prrentalminvalue").val($("#rentalminvalue"+ids).val());
...
<?php 
if($rowselect[0]['consigneeid']==$rowselect[0]['consigneename'])
{
?>
...
{
?>
$("#consigneeid").val('<?=$rowselect[0]['consigneeid']?>');
$("#chchconsigneename").val('<?=$rowselect[0]['consigneeid']?>');
...
//$("#consigneeid").val("<?=$rowselect[0]['consigneeid']?>").trigger("change");
...
var rentper=document.getElementById("rentper"+id);
var rentperamount=document.getElementById("rentperamount"+id);
...
rentper.value=gstp.toFixed(2);
gsta=((parseFloat(gstper.value)*parseFloat((productamt-discountamt)))/100);
gstamount.value=gsta.toFixed(2);
rentperamount.value=gsta.toFixed(2);
$("#gstresponse"+id).html('GST('+gstp+'%): '+gsta.toFixed(2));
