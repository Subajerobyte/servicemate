...
for ($i = 0; $i < count($data); $i++) {
	...
	for ($j = 0; $j < 39; $j++) {
		...
		if($j==11)
		{
			$arrconsigneename[]=mysqli_real_escape_string($connection,string_sanitize(trim($data[$i][$j])));;
			$consigneename=mysqli_real_escape_string($connection,string_sanitize(trim($data[$i][$j])));;
		}
		if($j==12)
		{
