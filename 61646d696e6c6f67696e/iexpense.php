...
<head>
  ...
  <title><?=$_SESSION['companyname']?> - Jerobyte - Expense Details</title>
 ...
 <?php include('accountnavbar.php');?>
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
		    ...
		    <div class="row">
  <div class="col">
    <h1 class="h4 mb-2 mt-2 text-black-800 text-center"><b>Expense Details</b></h1>
  </div>
  <div class="col-auto">
    <a href="iexpenseadd.php" class="m-2 btn btn-sm btn-primary shadow-sm"><i class="fas fa-plus fa-sm text-white-50"></i> Add New Expense</a>
  </div>
</div>
		  <?php
...
?>
			...
			<div class="floating-container"><div class="text-center mt-3"><a class="btn btn-scroll" id="scrollUpBtn" onmousedown="startContinuousScroll('up')" onmouseup="stopContinuousScroll()"><i class="fas fa-chevron-circle-up"></i></a><a class="btn btn-scroll" id="scrollLeftBtn" onmousedown="startContinuousScroll('left')" onmouseup="stopContinuousScroll()"><i class="fas fa-chevron-circle-left"></i></a><a class="btn btn-scroll" id="scrollRightBtn" onmousedown="startContinuousScroll('right')" onmouseup="stopContinuousScroll()"><i class="fas fa-chevron-circle-right"></i></a><a class="btn btn-scroll" id="scrollDownBtn" onmousedown="startContinuousScroll('down')" onmouseup="stopContinuousScroll()"><i class="fas fa-chevron-circle-down"></i></a></div></div>
              <div class="table-responsive scroll">
                <table class="table table-bordered font-13" id="dataTable" width="100%" cellspacing="0">
                  ...
                  <tbody>
				  <?php
				  $sqlselect = "SELECT edate, categoryname, amount, invoice, remarks, enabled, expensedoc, id From jrciexpense order by id desc";
        ...
        if($rowCountselect > 0) 
		{
			...
			{
				$sqlserial = "SELECT id, categoryname From jrcexpense where id='".$rowselect['categoryname']."'";
        $queryserial = mysqli_query($connection, $sqlserial);
        $rowCountserial = mysqli_num_rows($queryserial);
        if(!$queryserial){
           die("SQL query failed: " . mysqli_error($connection));
        }
        $rowcategory = mysqli_fetch_array($queryserial)
		
			?>
                    <tr>
                      <td><?=$count?></td>
                      <td><?=$rowselect['edate']?></td>
                      <td><?php if($rowCountserial>0){ echo $rowcategory['categoryname']; }?></td>
                      <td><?=$rowselect['amount']?></td>
                      <td><?=$rowselect['invoice']?></td>
                      <td><?=$rowselect['remarks']?></td>
					  <td data-label="Photo">
								  <?php 
									if($rowselect['expensedoc']!="")
									{
										?>										
										<a href="#" class="pop"><img src="<?php echo $rowselect['expensedoc']?>" width="100" height="100"></a>
										<?php
									}
									?>
								  &nbsp;</td>
                      <td><a href="iexpenseedit.php?id=<?=$rowselect['id']?>">Edit</a></td>
                      <td>
                      <?php
                      if($rowselect['enabled']=='0')
                      {
                      ?>
                      <a href="iexpensechange.php?id=<?=$rowselect['id']?>&val=1" onclick="return confirm('Are you sure want to Disable this Expense?')" ><span class="text-success">Enabled</span></a>
                      <?php						
                      }
                      else
                      {
                        ?>
                      <a href="iexpensechange.php?id=<?=$rowselect['id']?>&val=0" onclick="return confirm('Are you sure want to Enable this Expense?')" ><span class="text-danger">Disabled</span></a>
                      <?php
                      }
                      ?>
                      </td>
