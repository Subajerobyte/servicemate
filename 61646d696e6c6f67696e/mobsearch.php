...
$query = "SELECT mobile, id as consigneeid, department, subcategory, maincategory, department, address1, address2, area, pincode, contact, phone, consigneename, email, district, gstno, statecode , gsttype FROM jrcconsignee WHERE (".$q.") ".$a." group by mobile ORDER BY mobile ASC";
...
if(($encsystem=='1')||($encsystem=='2'))
{
	if((isset($encpass))&&($encpass!=""))
	{
		...
		{
		$row['consigneename']=jbsdecrypt($_SESSION['encpass'], $row['consigneename']);
		}
		if($row['email']!='')
		{
		...
		foreach($terms as $t)
		{
		if((strpos(strtolower($row['address1']), strtolower($t)) !== false)||(strpos(strtolower($row['phone']), strtolower($t)) !== false)||(strpos(strtolower($row['consigneename']), strtolower($t)) !== false)||(strpos(strtolower($row['email']), strtolower($t)) !== false))
		{
		$data[] = $row; 
		}
		}
...
if ((isset($_GET['term']))&&($_GET['term']!="")) {
 ...
 {
  $data['value'] = $maincategory['mobile'];
 }
 else
 {
