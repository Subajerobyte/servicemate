...
if(isset($_POST['submit']))
{
		...
		$engineersid=mysqli_real_escape_string($connection, $_POST['engineersid']);
		$callfromvalue=mysqli_real_escape_string($connection, $_POST['callfrom']);
		...
		if(isset($_POST['rd']))
		{
		$rd=mysqli_real_escape_string($connection, $_POST['rd']);
		}
		else
		{
		$rd='';	
	...
	{		
		...
		{	
			...
			else
			{
				...
				{
					 mysqli_query($connection, "update jrcgodowninventory set outworddate='$times' where callid='$id'");
					// for oem replaced the product update the serialnumber
					 if($supcompstatus=='Replaced' && $compstatusvalue=='2')
					{
						
						if($revserialnumber!='NUll' && $revserialnumber!='')
						{
							
							$sqlxl1=mysqli_query($connection, "select productid from jrcxl where id='$sourceidvalue'");
							$rowCountxl1 = mysqli_num_rows($query);
							if($rowCountxl1 > 0)
							{
						$rowxl1=mysqli_fetch_array($sqlxl1);
						$serialremarks='OEM_Replaced_Product - Old Serial : '.$serial;
						$oldserial=$serial;
						$success=mysqli_query($connection, "update jrcserials set serialnumber='$revserialnumber' where sourceid='$sourceidvalue' and consigneeid='$consigneeidvalue' and  serialnumber='$serial'");
						$success1=mysqli_query($connection, "update jrcxl set serialnumber='$revserialnumber' where id='$sourceidvalue' and consigneeid='$consigneeidvalue' and  serialnumber='$serial'");
						if($success && $success1)
						{
						 $sqlserialhis = "INSERT INTO jrcserialhistory( createdon, createdby, sourceid, consigneeid, productid, calltid, serialnumber, oldserialnumber, remarks) VALUES ( '$createdon', '$createdby','$sourceidvalue', '$consigneeidvalue', '".$rowxl1['productid']."', '$calltid', '$revserialnumber', '$oldserial', '$serialremarks')";
						$queryserialhis = mysqli_query($connection, $sqlserialhis);
							}
							}
					}
				}
		...
		if($customersmsvalue=="0")
			...
			if($compstatusvalue=='2')
			{
				...
				{
			        $mailsubject="Your ".$callnature." call Completed: Call ID ".$calltid;
                    $mailcontent="Dear Customer, Your ".$callnature." Call with Call ID ".$calltid." is Completed. Kindly Share Your Feedback to improve our Service. Thank You - '".$_SESSION['companyname']."'";
			...
			{
				 ////////////Sharp
             $custmessage="Dear Customer, Your ".$callnature." Call with Call ID ".$calltid." is Completed. Kindly Share Your Feedback to improve our Service. Thank You - Sharp";
					$custmessage=preg_replace('!\s+!', ' ', $custmessage);
					$custmessage=str_replace(',,', ',', $custmessage);
					$custmessage=urlencode($custmessage);
				...
				if($compstatusvalue=='1')
			   {
				...
				{
			        $mailsubject="Your ".$callnature." call Completed: Call ID ".$calltid;
                    $mailcontent="Dear Customer, Your ".$callnature." Call Id(".$calltid.")is Pending. Regret for the inconvenience, Will be Closed Shortly - ".$_SESSION['companyname'];
					$mailsendto=trim($row['email']);
					sendmail($_SESSION['mailer'], $_SESSION['companyname'], $mailsubject, $mailcontent, $_SESSION['mailname'], $_SESSION['apppassword'], $mailsendto);
				   }
				}
				...
				{
					////////////Sharp
					$custmessage="Dear Customer, Your ".$callnature." Call Id(".$calltid.")is Pending. Regret for the inconvenience, Will be Closed Shortly - Sharp";
					$custmessage=preg_replace('!\s+!', ' ', $custmessage);
					$custmessage=str_replace(',,', ',', $custmessage);
					$custmessage=urlencode($custmessage);
					$url = "http://www.nminfotech.in/smsautosend.aspx?id=SHARPC&PWD=SHARPC&mob=".$callfromvalue."&msg=".$custmessage."&tm=T";
        ...
        if($rowCounteng > 0) 
		{
			...
			while($roweng = mysqli_fetch_array($queryeng)) 
			{
				...
				/* Mail*/	
			        $mailsubject="Your ".$callnature." call Completed: Call ID ".$calltid;
					$mailcontent=$engmessage1="Dear Friend, Call ID: ".$calltid." ".$statitle;
				...
				{
					////////jrc
					$engmessage=$engmessage1="Dear Friend, Call ID: ".$calltid." ".$statitle;
				...
				{
					////////////uel
					$engmessage=$engmessage1="Dear Friend, Call ID: ".$calltid."".$statitle." -UEL";
					$engmessage=preg_replace('!\s+!', ' ', $engmessage);
					$engmessage=str_replace(',,', ',', $engmessage);
					$engmessage=urlencode($engmessage);
		...
		if($coordinatorsmsvalue=="0")
		{
			...
			if($rowCountcoor > 0) 
			{
                ...
                /* Mail*/	
			        $mailsubject="Your ".$callnature." call Completed: Call ID ".$calltid;
					$mailcontent=$coormessage1="Dear Co-ordinator, Call ID ".$calltid." ".$statitle;
					...
					{
						////////jrc
						$coormessage=$coormessage1="Dear Co-ordinator, Call ID ".$calltid." ".$statitle;
					...
					{
						////////////uel
						$coormessage=$coormessage1="Dear Co-ordinator, Call ID ".$calltid."".$statitle." -UEL";
						$coormessage=preg_replace('!\s+!', ' ', $coormessage);
						$coormessage=str_replace(',,', ',', $coormessage);
						$coormessage=urlencode($coormessage);
...
/*Notifications*/
		...
		if($rd!='')
		{
			if($rd=="open")
			{
				header("Location: calls.php?status=0&remarks=Updated Successfully");
			}
			else if($rd=="pending")
			{
				header("Location: calls.php?status=1&remarks=Updated Successfully");
			}
			else
			{
			header("Location: consigneeview.php?id=".$consigneeidvalue."&remarks=Updated Successfully");	
			}
