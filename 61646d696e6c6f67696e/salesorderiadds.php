...
if(isset($_GET['invoice']))
	 {
		...
		if($_GET['sotype']=='Regular')
		{
		$query = "UPDATE jrctally SET invoiceno='SJ / JRC / ".$currentMonthYear."/" . $row11['invoiceno'] . "' , invoicedate1='".$date."' WHERE sono='" . $_GET['id'] . "'";
		}
		else
		{
		 $query = "UPDATE jrctally SET invoiceno='SJ / SER / ".$currentMonthYear."/" . $row11['invoiceser'] . "' , invoicedate1='".$date."' WHERE sono='" . $_GET['id'] . "'";
		}
		$result = mysqli_query($connection, $query);
		
		if($_GET['sotype']=='Regular')
		{
		$query1 = "UPDATE jrcxl SET invoiceno='SJ / JRC / ".$currentMonthYear."/" . $row11['invoiceno'] . "' , invoicedate='".$date."' WHERE sono='" . $_GET['id'] . "'";
		}
		else
		{
		$query1 = "UPDATE jrcxl SET invoiceno='SJ / SER / ".$currentMonthYear."/" . $row11['invoiceser'] . "' , invoicedate='".$date."' WHERE sono='" . $_GET['id'] . "'";	
		}
...
if($rowCountselect > 0) 
{
	...
	mysqli_query($connection, "INSERT INTO jrchistory (user, ipaddress, times, remarks, tid, tablename) VALUES ('{$useremail}', '{$ip}', '{$times}', 'Uploaded from Sales Order', '', 'jrcxl')");
	 header("Location:compexporttally.php?remarks=Invoice Generated Successfully"); 	
		}
	 }
	 else
	 {
	 header("Location:exporttally.php?error=Invoice Not Generated"); 	
	 }
	 }
	 }
?>