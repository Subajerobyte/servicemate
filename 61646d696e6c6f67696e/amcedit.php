...
else
{
	echo '<span class="text-success"><strong>('.$effectiveDate1.')</strong></span>';
					  ...
					  <br>
					  <?php
				  $sqlamc = "SELECT dateto, datefrom, amcduration, amctype  From jrcamc where sourceid='".$rowselect['id']."' order by id desc";
	  ...
	  <br>
		...
		<div class="col-xl-12 order-xl-2 mb-5 mb-xl-0">
          <div class="card card-profile shadow">
            ...
            <div class="card-body pt-0 pt-md-4">
			<form action="amcedits.php" method="post" onload="valuefun()">
			
			<input type="hidden" name="consigneeid" value="<?=$_GET['consigneeid']?>">
			<input type="hidden" name="sourceid" value="<?=$_GET['xlid']?>">
			<input type="hidden" name="productid" value="<?=$productid?>">
			<?php 
$sqlselect1 = "SELECT DISTINCT stockitem, amcvalue,amcgst,gsttype From jrcproduct where id='".$productid."' order by id asc";
...
<?php
			...
			if(isset($_GET['i']))
			{
				$amc='and id="'.$_GET['i'].'"';
				?>
				<input type="hidden" name="amcid" id="amcid" value="<?=($_GET['i'])?>">
			...
			$today=date('Y-m-d');
				  $sqlamc = "SELECT amctype, amcduration, datefrom, dateto,quotationtype, amcdetails, priceperyear, quantity, amcgst,amcgstvalue, resultvalue, totalvalue,btotalvalue, serialnumber, receivedamount, receivedmode, receiveddate, receivedby,remarks,amcinvoiceno,amcrenewtype  From jrcamc where sourceid='".(mysqli_real_escape_string($connection, $_GET['xlid']))."'  $amc order by id desc";
...
<div class="col-lg-3">
	...
	{
	?>
	<input type="text" class="form-control" id="priceperyear" name="priceperyear" required value="<?=$rowamc['priceperyear']?>" onchange="valuefun();monthupdate()" >
	<?php
	}
	else
	{
		 $sqlxl = "SELECT * From jrcxl where id='".$_GET['xlid']."' ";
			...
			$sqlselect1 = "SELECT DISTINCT stockitem, amcvalue,amcgst From jrcproduct where id='".$rowxl['productid']."' order by id asc";
			$queryselect1 = mysqli_query($connection, $sqlselect1);
			$rowselect1 = mysqli_fetch_array($queryselect1);
				?>
...
<div class="col-lg-3">
	...
	<input type="date" class="form-control" id="datefrom" name="datefrom" required value="<?=$rowamc['datefrom']?>" onchange="monthupdate()" >
  </div>
  </div>
 <div class="col-lg-3">
    <div class="form-group">
  ...
  {	  ?>
	<select class="form-control fav_clr" name="serialnumber[]" id="serialnumber"  onchange="valuefun();qtyfun()" multiple>
	<option value="">Select</option>
	<?php
...
<div class="col-lg-3">
	...
	<input type="text" class="form-control" id="quantity" name="quantity"  value="<?=$rowamc['quantity']?>"  onchange="valuefun();qtyfun()"  required>
  </div>
  </div>
 <div class="col-lg-3">
    <div class="form-group">
    <label for="resultvalue">Value</label>
	<input type="number" class="form-control" id="resultvalue" name="resultvalue" required value="<?=$rowamc['resultvalue']?>" onchange="valuefun();calculateGSTValue()" step="0.01" readonly >
  </div>
  </div> 
<div class="col-lg-3">
    <div class="form-group">
	...
	{
	?>
	<input type="text" class="form-control" id="amcgst" name="amcgst" required value="<?=$rowamc['amcgst']?>" onchange="valuefun();calculateGSTValue()" >
	<?php
	}
	else
	{
		?>
		<input type="text" class="form-control" id="amcgst" name="amcgst" required  onchange="valuefun();calculateGSTValue()" >
		<?php
	}
	?>
  </div>
...
<div class="col-lg-3">
	...
	<input type="text" class="form-control" id="amcgstvalue" name="amcgstvalue" required value="<?=$rowamc['amcgstvalue']?>" readonly onchange="valuefun();calculateGSTValue()">
  ...
  <hr>
   ...
   <input type="text" class="form-control" name="remarks" id="remarks" value="<?=$rowamc['remarks']?>"></div>
...
else if(isset($_GET['qid']))
		{
				 ...
				 $sqlq = "SELECT maintenancetype, dateto, datefrom,noofmonths,qno,qdate,id,priceperyear,serialnumber,resultvalue,quantity,amcgst,amcgstvalue,totalvalue,btotalvalue,quotationtype,amcrenewtype  From jrcamcquotation where id='".(mysqli_real_escape_string($connection, $_GET['qid']))."'";
...
<div class="col-lg-3">
    <div class="form-group">
    <label for="quantity">Quantity</label>
	<input type="text" class="form-control" id="quantity" name="quantity"  value="<?=$rowq['quantity']?>"  onchange="valuefun();"  required>
  </div>
  </div>
 <div class="col-lg-3">
    <div class="form-group">
    ...
    <div class="form-group">
    <label for="amcgst">GST %</label>
	<input type="text" class="form-control" id="amcgst" name="amcgst"  value="<?=$rowq['amcgst']?>" onchange="valuefun();calculateGSTValue()"  required>
  </div>
  </div>
...
<div class="col-lg-3">
    <div class="form-group">
    <label for="amcgstvalue">GST Value</label>
	<input type="text" class="form-control" id="amcgstvalue" name="amcgstvalue"  value="<?=$rowq['amcgstvalue']?>" onchange="valuefun();calculateGSTValue()"  readonly required>
  </div>
  </div>
 <div class="col-lg-3">
    <div class="form-group">
...
<?php
		...
		//add new amc (expert amc customer)
		else
		{
...
<div class="col-lg-3">
 ...
 <div class="col-lg-3">
  ...
  <div class="col-lg-3">
    <div class="form-group">
    <label for="quantity">Quantity</label>
	<input type="text" class="form-control" id="quantity" name="quantity" onchange="valuefun();qtyfun()"   required >
  </div>
  </div> 
  <div class="col-lg-3">
    <div class="form-group">
  ...
  <div class="col-lg-3">
    <div class="form-group">
    <label for="amcgst">GST%</label>
	<input type="text" class="form-control" id="amcgst" name="amcgst" value="<?=$rowselect1['amcgst']?>" onchange="valuefun();calculateGSTValue()"  required >
  </div>
  </div> 
  <div class="col-lg-3">
    <div class="form-group">
...
function updateResult() {
  ...
  var amcduration = parseFloat(document.getElementById("amcduration").value) || 0;
  var quantity = document.getElementById("quantity").value;
  var amcgst = parseFloat(document.getElementById("amcgst").value) || 0;
  var gsttype = document.getElementById("gsttype").value;
if(quantity!='' && amcduration!='')
{
  var resultvalue = (priceperyear !="" && amcduration !="") ? ((priceperyear / 12) * amcduration) : priceperyear.toFixed(2);
...
function calculateGSTValue() {
    var gstPercentage = document.getElementById('amcgst').value;
    var gstValueField = document.getElementById('amcgstvalue');

    // Assuming you have a base amount from which the GST value is calculated
    var baseAmount = document.getElementById("priceperyear").value; // Replace this with the actual base amount or retrieve it dynamically
    var gstValue = (gstPercentage / 100) * baseAmount;

    gstValueField.value = gstValue.toFixed(2); // Update the GST value field
}
</script>
