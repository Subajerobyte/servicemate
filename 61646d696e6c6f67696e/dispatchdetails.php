...
?>
			<?php
		    $sqlselect = "SELECT dispatchdocno, disthrough From jrctally where sono='".$sono."' group by sono";
			...
			{
			?>
			        <form id="dcForm" action="salesorderdcadds.php" method="POST">
                    <input type="hidden" name="id" id="ewaySono" value="<?=$sono?>">
                    <input type="hidden" name="dc" id="dc" value="generate">
                  <div class="form-group">
				<label for="dispatchdocno">Dispatch Document No:</label>
				<input type="text" class="form-control" name="dispatchdocno" id="dispatchdocno" maxlength="15" value="<?=$rowselect['dispatchdocno']?>">
			</div>
