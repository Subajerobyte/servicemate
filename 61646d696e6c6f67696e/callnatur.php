...
function get_maincategory($connection , $term){ 
 $query = "SELECT callnature, id FROM jrccallnature WHERE LOWER(callnature) LIKE LOWER('%".$term."%') and enabled='0' group by callnature ORDER BY callnature ASC";
...
if (isset($_GET['term'])) {
 ...
 $data['id'] = $maincategory['id'];
  $data['value'] = $maincategory['callnature'];
