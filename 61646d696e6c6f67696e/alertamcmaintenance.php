...
<title><?=$_SESSION['companyname']?> - Jerobyte - AMC Maintenance Alert</title>
...
<div class="row">
<div class="col">
<h1 class="h4 mb-2 mt-2 text-black-800 text-center"><b>AMC Maintenance Alert</b></h1>
</div>
</div>
<?php
...
<?php
$soure="";
$sqlsels = "SELECT sourceid From jrcreminder where remindertype='AMC MAINTENANCE' and enabled='0' order by sourceid asc";
...
<tbody>
<?php
$sqlselect = "SELECT sourceid, createdon, reminder, enddate, status, id, enabled From jrcreminder where remindertype='AMC MAINTENANCE' and enabled='0' ".$sour." ".$staqu1." order by DATE(enddate) asc";
$queryselect = mysqli_query($connection, $sqlselect);
$rowCountselect = mysqli_num_rows($queryselect);
if(!$queryselect){
die("SQL query failed: " . mysqli_error($connection));
...
<td></td>
<td>
<a href="reminderchange.php?id=<?=$rowselect['id']?>&val=0" onclick="return confirm('Are you sure want to make Pending this AMC Maintenance Alert?')" ><span class="text-success">Completed</span></a>
</td>
<?php
}
else
{
?>
<td><a target="_blank" href="callsadd.php?id=<?=$rowxl['id']?>&ts=amcmaintenance&ts1=<?=$rowselect['id']?>" class="btn btn-primary btn-sm">Take Call</a></td>
<td>
<a href="reminderchange.php?id=<?=$rowselect['id']?>&val=1" onclick="return confirm('Are you sure want to Complete this AMC Maintenance Alert?')" ><span class="text-danger">Pending</span></a>
</td>
<?php
}
?>
