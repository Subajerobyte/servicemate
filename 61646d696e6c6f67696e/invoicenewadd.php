...
<head>
  ...
  <title><?=$_SESSION['companyname']?> - Jerobyte - Add New Invoice Details</title>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
		  ...
		  <div class="row">
  <div class="col">
    <h1 class="h4 mb-2 mt-2 text-black-800 text-center"><b>Add New Invoice Details</b></h1>
  </div>
  <!--<div class="col-auto">
...
$id=mysqli_real_escape_string($connection,$_GET['id']);
        ...
        if($rowCountselect > 0) 
		{
			...
			{ */
			?>
<form action="invoicenewadds.php" method="post">
...
{
?>
	                  <td><?=$rowselect1['consigneename']?></td>
					  <?php					  
}
if(($infolayoutcustomers['address1']=='1')||($infolayoutcustomers['address2']=='1')||($infolayoutcustomers['area']=='1')||($infolayoutcustomers['district']=='1')||($infolayoutcustomers['pincode']=='1'))
{
?>
					  <td><?=$rowselect1['address1']?> <?=$rowselect1['address2']?> <?=$rowselect1['area']?> <?=$rowselect1['district']?> <?=$rowselect1['pincode']?></td>
					  <?php					  
}
if(($infolayoutcustomers['contact']=='1')||($infolayoutcustomers['phone']=='1')||($infolayoutcustomers['mobile']=='1')||($infolayoutcustomers['email']=='1'))
{
?>
					  <td><?=$rowselect1['contact']?> <?=$rowselect1['phone']?> <?=$rowselect1['mobile']?> <?=$rowselect1['email']?></td>
					  <?php				  
}
?>
					  <td><a href="consigneeedit.php?id=<?=$_GET['id']?>">Edit</a></td>
                    </tr>
				  </tbody>
                </table>
              </div>
...
if ($infolayoutinvoice['invoiceno'] == '1') 
{
    ...
    if ($infosr = mysqli_fetch_array($querysr)) {
        $newInvoiceNo = (int)$infosr['nsino'] + 1;
        $invoiceno = $_SESSION['companyshortname'] . ' / NSI / ' . date('m') . date('y') . ' / ' . str_pad($newInvoiceNo, 5, "0", STR_PAD_LEFT);
        $invoicedate = date('Y-m-d');
        mysqli_query($connection, "UPDATE jrcsrno SET nsino = '$newInvoiceNo'");

        ?>
        <div class="col-lg-3">
            <div class="form-group">
                <label for="invoiceno">Invoice No </label>
                <input type="text" name="invoiceno" id="invoiceno" class="form-control" value="<?php echo $invoiceno; ?>" <?= ($infolayoutinvoice['invoicenoreq'] == '1') ? 'required' : ''; ?>>
            </div>
        </div>
        <?php
    }
...
{
	?>
	<input type="hidden" name="tenderno" id="tenderno" >
	<?php
}
...
{
	?>
	<input type="hidden" name="claimsubon" id="claimsubon" >
	<?php
}
...
{
	?>
	<input type="hidden" name="invoiceamt" id="invoiceamt" >
	<?php
}
...
<div class="col-lg-3">
    ...
    <input type="number" class="form-control" id="claimper" name="claimper" <?=($infolayoutinvoice['claimperreq']=='1')?'required':''?> onchange="amount()" max="100" min="0">
  </div>
  </div>
 <?php
}
...
{
	?>
	<input type="hidden" name="claimper" id="claimper" >
	<?php
}
...
{
	?>
	<input type="hidden" name="claimamt" id="claimamt" >
	<?php
}
...
{
	?>
	<input type="hidden" name="installrefno" id="installrefno" >
	<?php
}
...
{
	?>
	<input type="hidden" name="suprefno" id="suprefno" >
	<?php
}
...
{
	?>
	<input type="hidden" name="dcno" id="dcno" >
	<?php
}
...
{
	?>
	<input type="hidden" name="dcdate" id="dcdate">
	<?php
}
...
{
	?>
	<input type="hidden" name="installedon" id="installedon" >
	<?php
}
...
{
	?>
	<input type="hidden" name="installedby" id="installedby" >
	<?php
}
?>


 <input type="hidden" name="id" id="id" value="<?=$rowselect1['id']?>">
  <input type="hidden" name="maincategory" id="maincategory" value="<?=$rowselect1['maincategory']?>">
  <input type="hidden" name="subcategory" id="subcategory" value="<?=$rowselect1['subcategory']?>">
  <input type="hidden" name="consigneename" id="consigneename" value="<?=$rowselect1['consigneename']?>">
  <input type="hidden" name="department" id="department" value="<?=$rowselect1['department']?>">
  
  <input type="hidden" name="address1" id="address1" value="<?=$rowselect1['address1']?>">
  <input type="hidden" name="address2" id="address2" value="<?=$rowselect1['address2']?>">
  <input type="hidden" name="area" id="area" value="<?=$rowselect1['area']?>">
  <input type="hidden" name="district" id="district" value="<?=$rowselect1['district']?>">
  <input type="hidden" name="pincode" id="pincode" value="<?=$rowselect1['pincode']?>">
  <input type="hidden" name="contact" id="contact" value="<?=$rowselect1['contact']?>">
  <input type="hidden" name="phone" id="phone" value="<?=$rowselect1['phone']?>">
  <input type="hidden" name="mobile" id="mobile" value="<?=$rowselect1['mobile']?>">
  <input type="hidden" name="email" id="email" value="<?=$rowselect1['email']?>">
<?php
...
{
	?>
	<input type="hidden" name="invoicedqty" id="invoicedqty">
	<?php
}
...
{
	?>
	<input type="hidden" name="overallwarranty" id="overallwarranty" >
	<?php
}
?>
...
?>					  
                  ...
                  <tbody>
				  <?php
				 /*  $sqlselect = "SELECT id, productid, serialnumber, stockmaincategory, stocksubcategory, stockitem, typeofproduct, componenttype, componentname, make, capacity, warranty, qty From jrcxl where tdelete='0' order by stockitem asc, typeofproduct desc";
		...
		$count=1;
			?>
			<tr>	
<input type="hidden" name="oldxlid[]" id="oldxlid<?=$count?>" >
<input type="hidden" name="productid[]" id="productid<?=$count?>" >
<input type="hidden" name="serialnumber[]" id="serialnumber<?=$count?>" >	
...
{
	?>
	<input type="hidden" name="stockmaincategory[]" id="stockmaincategory<?=$count?>" >
	<?php
}
if($infolayoutproducts['stocksubcategory']=='1')
{
...
<?php
	...
	{
		?>
		<td style="width:<?=$cco/100?>%"><textarea rows="6" name="stocksubcategory[]" id="stocksubcategory<?=$count?>"  class="form-control"  <?=($infolayoutproducts['stocksubcategoryreq']=='1')?'required':''?>></textarea></td>
		<?php
	}
}
else
{
	?>
	<input type="hidden" name="stocksubcategory[]" id="stocksubcategory<?=$count?>" >
	<?php
}
if($infolayoutproducts['stockitem']=='1')
{
...
{
	?>
	<input type="hidden" name="stockitem[]" id="stockitem<?=$count?>" >
	<?php
}
if($infolayoutproducts['typeofproduct']=='1')
{
...
{
	?>
	<input type="hidden" name="typeofproduct[]" id="typeofproduct<?=$count?>" >
	<?php
}
if($infolayoutproducts['componenttype']=='1')
{
...
{
	?>
	<input type="hidden" name="componenttype[]" id="componenttype<?=$count?>" 
	<?php
}
if($infolayoutproducts['componentname']=='1')
{
...
{
	?>
	<input type="hidden" name="componentname[]" id="componentname<?=$count?>" >
	<?php
}
if($infolayoutproducts['make']=='1')
{
...
{
	?>
	<input type="hidden" name="make[]" id="make<?=$count?>" >
	<?php
}
if($infolayoutproducts['capacity']=='1')
{
...
{
	?>
	<input type="hidden" name="capacity[]" id="capacity<?=$count?>" >
	<?php
}
if($infolayoutinvoice['warranty']=='1')
{
...
{
	?>
	<input type="hidden" name="warranty[]" id="warranty<?=$count?>" >
	<?php
}
if($infolayoutinvoice['qty']=='1')
{
?>					  
              <td style="width:<?=$cco/100?>%">
			  <input type="number" name="qty[]" id="qty<?=$count?>"  class="form-control" <?=($infolayoutinvoice['qtyreq']=='1')?'required':''?> ></td>
<?php
}
else
{
	?>
	<input type="hidden" name="qty[]" id="qty<?=$count?>" >
	<?php
}
?>
         </tr>
