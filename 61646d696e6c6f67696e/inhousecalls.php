...
<head>
  ...
  <title><?=$_SESSION['companyname']?> - Jerobyte - Calls</title>
...
if(isset($_GET['call']))
{
	...
	else if($_GET['call']=='assinged')
		{
		$statitle="Assigned";
		if($staqu!="")
		{
			$staqu.=" and engineerid!='' ";
		}
		else
		{
			$staqu.=" where engineerid!=''";
		}	
		}
}
?>
...
<div class="row">
    <div class="col">
        <h1 class="h4 mb-2 mt-2 text-black-800 text-center" style="padding-left:250px;"><b><?=$statitle?> Calls</b></h1>
    </div>
            ...
            <div class="input-group">
                ...
                <div class="input-group-append">
                    ...
                    <button class="btn btn-navb d-inline-block" type="submit">
                        <a href="inhousecalls.php"><i class="fas fa-undo fa-sm" style="color:#3d8eb9;"></i></a>
                    </button>
                </div>
            </div>
        </form>
		  ...
		  else
		  {
			  if(isset($_GET['status']))
			{
				...
				else if($_GET['status']=='0')
				{
					...
					{
						$dashfromdate=date('Y-01-01');
								  $dashtodate=date('Y-12-t');
					}
				}
				else{
					$dashfromdate=date('Y-m-d');
			...
			{
				if($_GET['call']=='received' || $_GET['call']=='assinged')
				{
					$dashfromdate=date('Y-m-d');
					$dashtodate=date('Y-m-d');
				}
...
<div class="col-xl-7 col-md-7 mb-4">
            <div class="card bg-white text-primary shadow" role="button" >
                <div class="card-statistic-3 p-3">
                    <div class="row align-items-center  d-flex" style="font-size:14px;">
						...
						<div class="col-xl-4 col-md-4 mb-1" style="padding:0.25rem; z-index:2;">
								<div class="card bg-primary text-white shadow1 card1" role="button">
									<div class="card-statistic-3 p-1">
										<div class="row align-items-center  d-flex" style="font-size:14px;">
											<div class="col-12 text-center">
												<h5 class="card-title mb-0" style="font-size:1.2rem; line-height:1.5; font-weight:bold" onClick="window.location.href= 'inhousecalls.php?<?=(isset($_GET['status']))?'status='.(mysqli_real_escape_string($connection, $_GET['status'])):''?><?=(isset($_GET['status']))?'&ctype='.$rowselect['calltype']:'ctype='.$rowselect['calltype']?>'"><?=strtoupper($rowselect['calltype']).'S'?> <br><span style="font-size:1.5rem;"><?=$rowselect['count']?></span></h5>
											</div>
										</div>
									</div>
						...
						$sqlselect1 = "SELECT count(id) as count, callnature From jrccalls ".$staqu1." group by callnature order by count desc";
							...
							{
							?>
							<div class="col-xl-3 col-md-3 mb-1" style="padding:0.25rem; z-index:2;">
								<div class="card bg-primary text-white shadow1 card1" role="button">
									<div class="card-statistic-3 p-1">
										<div class="row align-items-center  d-flex" style="font-size:14px;">
											<div class="col-12 text-center" onClick="window.location.href= 'inhousecalls.php?<?=(isset($_GET['status']))?'status='.(mysqli_real_escape_string($connection, $_GET['status'])):''?><?=(isset($_GET['status']))?'&ctype='.$rowselect['calltype']:'ctype='.$rowselect['calltype']?><?=(isset($_GET['status']))?'&nature='.$rowselect1['callnature']:'&nature='.$rowselect1['callnature']?>'">
												<h5 class="card-title mb-0" style="font-size:0.7rem; line-height:1.5; font-weight:bold"><?=strtoupper($rowselect1['callnature'])?> <br><span style="font-size:1.5rem;"><?=$rowselect1['count']?></span></h5>
											</div>
										</div>
									</div>
									<div class="go-corner" href="#"><div class="go-arrow">→</div></div>
							...
							<?php
							${'tpy'.$t1}[]=strtoupper($rowselect1['callnature']);
							${'tpv'.$t1}[]=$rowselect1['count'];
								}
								?>
								</div>
								<?php
		...
		<div class="card bg-white text-primary shadow" role="button" style="height:100%;" >
		<canvas id="myChart<?=$t1?>" style="width:100%; height:100%"></canvas>
		</div>
		</div>
		<?php
		$t1++;
...
{
?>
                                    <h6 class="m-0 font-weight-bold text-dark">Pending Reasons</h6>
<div class="row">
<div class="col-xl-12 col-md-7 mb-4">
            <div class="card bg-white text-primary shadow" role="button" >
                <div class="card-statistic-3 p-3">
                    <div class="row align-items-center  d-flex" style="font-size:14px;">
                        <div class="col-12 text-center">
        ...
        if($rowCountselect > 0) 
		{
			...
			<div class="col-xl-2 col-md-2 mb-4">
								<div class="card bg-primary text-white shadow1 card1" role="button">
									<div class="card-statistic-3 p-3">
										<div class="row align-items-center  d-flex" style="font-size:14px;">
							<div class="col-12 text-center" onClick="window.location.href= 'inhousecalls.php?<?=(isset($_GET['xlid']))?'status='.(mysqli_real_escape_string($connection, $_GET['xlid'])):''?><?=(isset($_GET['status']))?'&action='.$rowselect['actiontaken']:'action='.$rowselect['actiontaken']?>'">								
		...
		?>
                    <tr>
					 ...
					 <td>
						 ...
						 <b>Estimate :</b> <span class="text-primary"><?=$rowselect['diagnosisestcharge']?></span><br>
...
if($rowselect['dcno']!='' && $rowselect['dcdate']!='')
{
                ...
                <b>DN Date:</b> <span class="text-primary"><?=($rowselect['dcdate']!='')?date('d/m/Y',strtotime($rowselect['dcdate'])):''?></span><br>
...
<script src="../../1637028036/vendor/jquery-ui/jquery-ui.min.js"></script><script>
<?php
for($at=1;$at<$t1;$at++)
{
	?>
