...
<head>
  ...
  <title><?=$_SESSION['companyname']?> - Jerobyte - Edit Complaint Call</title>
...
<div class="d-sm-flex align-items-center justify-content-between mb-1">
            <h1 class="h4 mb-2 mt-2 text-gray-800">Edit  Complaint Call</h1>
...
<?php	
			...
			<?php
					  ...
					  {
						  ?>
					  <a href="invoicedelete.php?consigneeid=<?=$rowselect['id']?>&xlid=<?=$infoselect2['id']?>&tdelete=<?=$infoselect2['tdelete']?>" onclick="return confirm('Are you sure want to Delete this Product')" class="button8"><i class="fa fa-trash"></i></a>
					  <?php
					  }
					  ?>
...
?>
			  ...
			  <form action="callsedits.php" method="post" onsubmit="return checkvalidate()">
...
<div class="col-lg-3">
  <div class="form-group">
    ...
    <input type="text" inputmode="numeric" class="form-control" id="callfrom" name="callfrom"  value="<?=$rowcalls['callfrom']?>" readonly <?=($infolayoutcall['callfrom']=='1')?'required':''?>>
  </div>
</div>
<?php
}
...
<div class="col-lg-3">
	...
	<input type="text" class="form-control" id="problemobserved" name="problemobserved" readonly value="<?=($rowcalls['problemobserved']!='')?$rowcalls['problemobserved']:(($rowcon4 >0 && ($infocon4['problemobserved']!=''))?$infocon4['problemobserved']:"")?>">
  </div>
  </div>
<?php
}
...
{
	?>
	<input type="hidden" name="problemobserved" id="problemobserved" value="<?=($rowcalls['problemobserved']!='')?$rowcalls['problemobserved']:(($rowcon4 >0 && ($infocon4['problemobserved']!=''))?$infocon4['problemobserved']:"")?>">
	<?php
}
...
<div class="col-lg-3">
  <div class="form-group">
  <label for="diagnosissigndemode">Delivery Mode</label>
   <select class="form-control fav_clr" id="diagnosissigndemode" name="diagnosissigndemode" value="<?=$rowcalls['diagnosissigndemode']?>">
...
<option value="By Courier">By Courier</option>
  ...
  <div class="col-lg-3">
  <div class="form-group">
  <label for="diagnosissigndemoderemark">Delivery Mode Remarks</label>
   <textarea  class="form-control" id="diagnosissigndemoderemark" name="diagnosissigndemoderemark" rows="2"  value="<?=$rowcalls['diagnosissigndemoderemark']?>">
</textarea>
  </div>
	</div>
	 <div class="col-lg-3">
...
<textarea class="form-control" id="diagnosismaterial" name="diagnosismaterial" readonly><?=$rowcalls['diagnosismaterial']?></textarea>
	...
	<?php
	$sqlgo = "SELECT id, godownname From jrcgodown where id='".$rowcalls['godownname']."'order by id asc";
        $querygo = mysqli_query($connection, $sqlgo);
        $rowCountgo = mysqli_num_rows($querygo);
		$rowgo = mysqli_fetch_array($querygo);
	?>
	<input type="text" class="form-control" id="godownname" name="godownname" readonly  value="<?=($rowcalls['godownname']!='')?$rowgo['godownname']:''?>">
...
<?php
$sqlsup = "SELECT id, suppliername From jrcsuppliers where id='".$rowcalls['suppliername']."' order by suppliername asc";
        $querysup = mysqli_query($connection, $sqlsup);
        $rowCountsup = mysqli_num_rows($querysup);
		$rowsup = mysqli_fetch_array($querysup);
        ?>
		<input type="text" class="form-control" id="godownname" name="godownname" readonly value="<?=($rowcalls['suppliername']!='')?$rowsup['suppliername']:''?>">
</select>
  </div>
  </div>
...
?>  
  <div class="col-lg-3">
    ...
    <input type="text" class="form-control" id="servicereportno" name="servicereportno" value="<?=($rowCountcalls >0 && $rowcalls['servicereportno']!='')?$rowcalls['servicereportno']:(($rowcon4 >0 && $infocon4['srno']!='')?$infocon4['srno']:"")?>" required>
  </div>
  </div>
<?php
}
...
<div class="col-lg-3">
    ...
    <input type="text" class="form-control" id="estimatedcost" name="estimatedcost" value="<?=( $rowCountcalls >0 && $rowcalls['estimatedcost']!='')?$rowcalls['estimatedcost']:($rowcon4 >0 && ($infocon4['scharge']!='')?$infocon4['scharge']:"")?>">
  </div>
  </div>
 <?php
}
...
?> 
  <div class="col-lg-3">
    ...
    <input type="text" class="form-control" id="materialtracking" name="materialtracking"  value="<?=($rowCountcalls >0 && $rowcalls['materialtracking']!='')?$rowcalls['materialtracking']:((($rowcon4 >0 && $infocon4['sparesused1']!='')?$infocon4['sparesused1']:"").(( $rowcon4 >0 && $infocon4['sparesused2']!='')?', '.$infocon4['sparesused2']:"").(($rowcon4 >0 && $infocon4['sparesused3']!='')?', '.$infocon4['sparesused3']:"").(($rowcon4 >0 && $infocon4['sparesused4']!='')?', '.$infocon4['sparesused4']:"").(( $rowcon4 >0 && $infocon4['sparesused5']!='')?', '.$infocon4['sparesused5']:""))?>">
  </div>
  </div>
 <?php
}
...
$sqlrep = "SELECT actiontaken From jrcactiontaken order by actiontaken asc";
        ...
        if($rowCountrep > 0) 
		{
			...
			{
				?>
<option value="<?=$rowrep['actiontaken']?>"  <?=($rowCountcalls >0 && $rowcalls['actiontaken']!='')?(( $rowCountcalls >0 && ($rowcalls['actiontaken']==$rowrep['actiontaken']))?'selected':''):(($rowcon4 >0 && ($infocon4['actiontaken']!=''))?(( $rowcon4 >0 && ($infocon4['actiontaken']==$rowrep['actiontaken']))?'selected':''):"")?>><?=$rowrep['actiontaken']?></option>
<?php
			}
		}
		?>
...
?> 
   <div class="col-lg-3">
	...
	<option value="1" <?=($rowCountcalls >0 && ($rowcalls['compstatus']=="1"))?"selected":(( $rowcon4 >0 && ($infocon4['callstatus']=='Pending'))?"selected":"")?> style="color:#e74a3b" >Pending</option>
	<option value="2" <?=($rowCountcalls >0 && ($rowcalls['compstatus']=="2"))?"selected":(($rowcon4 >0 && ($infocon4['callstatus']=='Completed'))?"selected":"")?> style="color:#1cc88a" >Closed</option>
	<option value="3" <?=($rowCountcalls >0 && ($rowcalls['compstatus']=="3"))?"selected":(($rowcon4 >0 && ($infocon4['callstatus']=='Cancelled')?"selected":""))?> style="color:#ebb434" >Cancelled</option>
	</select>
  </div>
  </div>
 <?php
...
<div class="col-lg-6">
    ...
    <textarea type="text" class="form-control" id="narration" name="narration" style="text-transform:uppercase" required ><?=($rowCountcalls >0 && ($rowcalls['narration']!=''))?$rowcalls['narration']:(($rowcon4 >0 && ($infocon4['engineerreport']!=''))?$infocon4['engineerreport']:"")?></textarea>
  </div>
  </div>
 <?php
}
...
{
	?>
	<input type="hidden" name="narration" id="narration" value="<?=($rowcalls['narration']!='')?$rowcalls['narration']:(($rowcon4 >0 && ($infocon4['engineerreport']!=''))?$infocon4['engineerreport']:"")?>">
	<?php
}
...
<input class="btn btn-primary" type="submit" name="submit" value="Approve & Submit">
  ...
  {
	  ?>
	  <div class="bg-danger text-white">Service Call Report is not Submitted by Engineer! Kindly inform to the Engineer to Make Service Call Report</div>
	  <br>
	  <?php
