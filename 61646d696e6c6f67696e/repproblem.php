...
function get_maincategory($connection , $term){ 
 $query = "SELECT reportedproblem, id FROM jrcreportedproblem WHERE LOWER(reportedproblem) LIKE LOWER('%".$term."%') and enabled='0' group by reportedproblem ORDER BY reportedproblem ASC";
...
if (isset($_GET['term'])) {
 ...
 $data['id'] = $maincategory['id'];
  $data['value'] = $maincategory['reportedproblem'];
