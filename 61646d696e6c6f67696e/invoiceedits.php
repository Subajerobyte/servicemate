...
else
{
	$warrantydate='';
                ...
                if ($oldxlidvalue == '') {
                    ...
                    $sourceid = $oldxlidvalue;
                    $sqlup = "update jrcxl set invoiceno= '{$invoiceno}', invoicedate= '{$invoicedate}', tenderno= '{$tenderno}', claimsubon= '{$claimsubon}', claimper= '{$claimper}', claimamt= '{$claimamt}', invoiceamt= '{$invoiceamt}', installrefno= '{$installrefno}', suprefno= '{$suprefno}', pono= '{$pono}', podate= '{$podate}', dcno= '{$dcno}', dcdate= '{$dcdate}', installedon= '{$installedon}', installedby= '{$installedby}', stockmaincategory = '{$stockmaincategory}', stocksubcategory = '{$stocksubcategory}', stockitem = '{$stockitem}', invoicedqty= '{$invoicedqty}', overallwarranty= '{$overallwarranty}', componenttype = '{$componenttype}', componentname = '{$componentname}', typeofproduct = '{$typeofproduct}', make= '{$make}', capacity= '{$capacity}', warranty= '{$warranty}', qty= '{$qty}', serialnumber= '{$serialnumber}' WHERE id = '{$sourceid}'";
                    $queryup = mysqli_query($connection, $sqlup);
        ...
        if ($rowCountselect > 0) {
            ...
            while ($rowselect = mysqli_fetch_array($queryselect)) {
                ...
                $sqlcon = "delete From jrcserials WHERE sourceid = '{$sourceid}'";
                $querycon = mysqli_query($connection, $sqlcon);
        ...
        mysqli_query($connection, "INSERT INTO jrchistory (user, ipaddress, times, remarks, tid, tablename) VALUES ('{$useremail}', '{$ip}', '{$times}', 'Invoice Updated', '{$sourceid}', 'jrcxl') ");
       header("Location: invoiceedit.php?id={$mainid}&remarks=Updated Successfully");
    } else {
