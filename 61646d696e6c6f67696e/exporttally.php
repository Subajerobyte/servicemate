...
<div class="row">
  <div class="col">
    <h1 class="h4 mb-2 mt-2 text-gray-800 text-center">Pending Sales Order</h1>
  </div>
</div>
...
$sqlselect = "SELECT sotype, consgst, conigst, concgst, grandtotal, contotal, consigneename, conaddress1, conaddress2, conaddress3, contaluk, condistrict, conpincode, duedays, dcno, sono, pono, podate, dname, createdon, invoicenofrom, invoicenoto, exp, invoiceno, installedon, expinvoice, expdc, irn, ewbno, canceldocumentstatus, ewbstatus, ewaypdf, invoicetype, expeinvoice, dispatchdocno, condepartment, DATE_ADD(podate, INTERVAL duedays DAY) AS next_due_date FROM jrctally ".$st." invoiceno = '' OR invoiceno IS NULL group by sono ORDER BY id desc";
$queryselect = mysqli_query($connection, $sqlselect);
$rowCountselect = mysqli_num_rows($queryselect);
if(!$queryselect){
die("SQL query failed: " . mysqli_error($connection));
...
if ($remainingDueDaysCeiled < 0) {
	...
	$ldamound=0;
$ldamound=$rowselect['grandtotal']/100*.25;
echo 'Invoice Amount - <b>'.  $rowselect['grandtotal'].'</b><br>';
...
{
?>
<a href="tallyedit.php?id=<?=$rowselect['sono']?>" style="background-color: #AFAFAF;" class="btn btn-sm shadow-sm text-white">Edit</a>
<?php
}
?>
</td>
...
if($rowselect['dcno']=='')
{
?><a href="javascript:void(0);" data-toggle="modal" data-target="#serialmodal" style="background-color: #AFAFAF;" class="btn btn-sm shadow-sm text-white" onclick="openModal('<?=$rowselect['sono']?>')">Update Serial No</a>
<a href="javascript:void(0);" data-toggle="modal" data-target="#dcModal"  onclick="openModal3('<?=$rowselect['sono']?>')" style="background-color: #9F5EB3;" class="btn btn-sm shadow-sm text-white">Generate</a><br>



<!--a href="salesorderdcadds.php?id=<?=$rowselect['sono']?>&dc=generate" style="background-color: #9F5EB3;" class="btn btn-sm shadow-sm text-white" >Generate</a-->
<br>
<td></td>
<?php
}
...
<a href="salesorderdc.php?id=<?=$rowselect['sono']?>" target="_blank" style="background-color: #00B3BC;" class="btn btn-sm shadow-sm text-white">View</a><br><a href="#" style="background-color: #AFAFAF;"  class="btn btn-sm shadow-sm text-white printLink" data-sono="<?php  $sono = $rowselect['sono']; echo $sono;  ?>">Print</a><br>
<?php
...
if($rowselect['invoiceno']=='')
{
if($rowselect['dispatchdocno']!='')
{
?>
<a href="salesorderiadds.php?id=<?=$rowselect['sono']?>&invoice=generate&sotype=<?=$rowselect['sotype']?>" style="background-color: #9F5EB3;" class="btn btn-sm shadow-sm text-white">Generate</a><br>
<?php
}
else
{
?>
<a href="javascript:void(0);" data-toggle="modal" data-target="#invModal"  onclick="openModal4('<?=$rowselect['sono']?>')" style="background-color: #9F5EB3;" class="btn btn-sm shadow-sm text-white">Generate</a><br>
<?php
}
}
else
{
	...
	<a href="salesorderinvoice.php?id=<?=$rowselect['sono']?>" target="_blank" style="background-color: #00B3BC;" class="btn btn-sm shadow-sm text-white">View</a><br><a href="#" style="background-color: #AFAFAF;"  class="btn btn-sm shadow-sm text-white printLinkin" data-sono="<?php   $sono = $rowselect['sono']; echo $sono; ?>">Print</a><br>
	<?php
...
<div class="modal" id="dcModal" tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
			...
			<div class="modal-body" id="dcContent">
...
<div class="modal" id="invModal" tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Dispatch Details</h5>
			...
			<div class="modal-body" id="invContent">
...
function openModal2(sono) {
	...
	function openModal3(sono) {
		
        // Send an AJAX request to your PHP script
        $.ajax({
            type: 'POST',
            url: 'dispatchdetails.php',
            data: { sono: sono },
            success: function (response) {
                // Handle the response from the server
                $('#dcContent').html(response);
               showAdditionalFields();
                // After the form submission, show the modal
                $('#dcModal').modal('show');
            },
	...
	function openModal4(sono) {
		
        // Send an AJAX request to your PHP script
        $.ajax({
            type: 'POST',
            url: 'dispatch1details.php',
            data: { sono: sono },
            success: function (response) {
                // Handle the response from the server
                $('#invContent').html(response);
               showAdditionalFields();
                // After the form submission, show the modal
                $('#invModal').modal('show');
            },
    ...
    function showAdditionalFields() {
        ...
        var vehicleNoInput = document.getElementById("etransportid");
...
<script>
$(document).on('click', '.printLinkic', function() {
        var sono = $(this).data('sono');
        $.ajax({
...
<script>
$(document).on('click', '.printLinkin', function() {
        var sono = $(this).data('sono');
        $.ajax({
...
<script>
$(document).on('click', '.printLinkein', function() {
        var sono = $(this).data('sono');
        $.ajax({
