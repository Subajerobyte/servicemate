...
if($installedonvalue!='')
{
$overdate=$installedonvalue;
}
else
{
$overdate= $invoicedatevalue;
}
if($warrantyvalue!='')
{
$off=(float)$warrantyvalue;
...
else
{
 ...
 $rowcopro = mysqli_fetch_array($querycon);
...
$sqlup = "update jrcxl set productid='{$productid}',warrantycycle='{$warrantycycle}',productlifetime='{$productlifetime}',lifetimedate='{$lifetimedate}'  WHERE stockmaincategory = '{$stockmaincategoryvalue}' and stocksubcategory = '{$stocksubcategoryvalue}' and stockitem = '{$stockitemvalue}' and componenttype = '{$componenttypevalue}' and componentname = '{$componentnamevalue}' and typeofproduct = '{$typeofproductvalue}' and make = '{$makevalue}' and capacity = '{$capacityvalue}' ";
$queryup = mysqli_query($connection, $sqlup);
if(!$queryup){
die("SQL query failed: " . mysqli_error($connection));
}
...
{
header("Location: consigneeview.php?id=".$consigneeidvalue."&error=Already A Complaint is Open for this Customer and Product! Kindly Close it");
}
}
}
else
