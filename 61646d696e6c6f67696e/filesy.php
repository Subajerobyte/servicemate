...
<?php
  if((isset($_GET['t']))&&(($_GET['t']=='Expense')))
  {
	    $sqlselect = "SELECT YEAR(edate) as edate From jrciexpense WHERE expensedoc!='' order by edate asc";
				 ...
				 else
		{
			$rowselect=array();
			   ...
			   $uniqueArray = array_map('unserialize', array_unique(array_map('serialize', $rowselect)));
				...
				?>
				<a href="filemanager.php" class="btn btn-primary">Back</a>
				<div class="row">
                <?php
		   ...
		   <img src="img/Folder.jpg" style="width:100px;height:100px;"></img><br>
		   <a href="filesm.php?t=Expense&y=<?=$row['edate']?>"><?=$row['edate']?></a>
		   </div>
		   <?php
 ...
 if((isset($_GET['t']))&&(($_GET['t']=='Calls')))
  { 
	 $sqlselect1 = "SELECT YEAR(callon) as callon From jrccalls WHERE diagnosisimg!='' or diagnosissignature!='' order by callon asc";
				 ...
				 else
		{
		   ...
		   <a href="filesm.php?t=Calls&y=<?=$row['callon']?>"><?=$row['callon']?></a>
		   </div>
		   <?php
 ...
 if((isset($_GET['t']))&&(($_GET['t']=='attendance')))
  {
	    $sqlselect5 = "SELECT YEAR(attdate) as attdate From jrcengroute WHERE tickets!='' order by attdate asc";
	   ...
	   <a href="filemanager.php" class="btn btn-primary">Back</a>
		   ...
		   <div class="col-lg-1 text-center">
		   <img src="img/Folder.jpg" style="width:100px;height:100px;"></img>
		   <a href="filesm.php?t=attendance&y=<?=$row['attdate']?>"><?=$row['attdate']?></a>
		   </div>
		   <?php
...
if((isset($_GET['t']))&&(($_GET['t']=='complaint')))
  { 
	   $sqlselect8 = "SELECT YEAR(addedon) as addedon From jrccalldetails WHERE imguploads!='' or imgbefuploads!='' order by addedon asc";   
       ...
       else
		{
		  ...
		  <a href="filemanager.php" class="btn btn-primary">Back</a>
		   ...
		   <div class="col-lg-1 text-center">
		   <img src="img/Folder.jpg" style="width:100px;height:100px;"></img>
		   <a href="filesm.php?t=complaint&y=<?=$row['addedon']?>"><?=$row['addedon']?></a>
		   </div>
		   <?php
