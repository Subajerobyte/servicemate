...
{
	$acknowledgementvalue=mysqli_real_escape_string($connection, $_POST['terms']);
		$msg = "";
  $msg_class = "";
	if(($acknowledgementvalue!=""))
	{		
		 
        $sqlcon = "SELECT id From jrcacknowledgement";
		...
		{	
			...
			$sqlup = "INSERT INTO jrcacknowledgement( acknowledgement) VALUES ( '$acknowledgementvalue')";
			...
			else
			{
				...
				mysqli_query($connection, "INSERT INTO jrchistory (user, ipaddress, times, remarks, tid) VALUES ('{$useremail}', '{$ip}', '{$times}', 'Insert A Acknowledgement Terms and Conditions', '{$tid}')");
				header("Location: acknowledgementsettings.php?remarks=Added Successfully");
			} 
	    }
		else
			{
				$sqlup = "update jrcacknowledgement set acknowledgement='$acknowledgementvalue'";
			$queryup = mysqli_query($connection, $sqlup);
			...
			else
			{
				...
				header("Location: acknowledgementsettings.php?remarks=Updated Successfully");
			}
			}
	}
	else
			{
				header("Location: acknowledgementsettings.php?error=Error Data");
			}
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title><?=$_SESSION['companyname']?> - Jerobyte - Acknowledgement Receipt Settings</title>
...
<!--<div class="d-sm-flex align-items-center justify-content-between mb-1">
            <h1 class="h4 mb-2 mt-2 text-gray-800">Acknowledgement Receipt Settings</h1>
          </div>-->
		  
		  	  
		  <div class="row">
  <div class="col">
    <h1 class="h4 mb-2 mt-2 text-black-800 text-center" ><b>Acknowledgement Receipt Settings</b></h1>
  </div>
  </div>
		  <?php
...
<div class="col-lg-12">
	<div class="form-group">
		<label for="terms">Terms and Conditions</label>
			<textarea id="terms" name="terms"><?=$info['acknowledgement']?></textarea>
	</div>	
</div>
