...
if(isset($_POST['submit']))
{
  ...
  $taxablevalue=mysqli_real_escape_string($connection, $_POST['taxablevalue']);
	...
	if(mysqli_num_rows($sqlis)>0)
	{
		...
		$sqlise=mysqli_query($connection, "update jrccalls set dcno='$dcno', dcdate='$dcdate', suppliername='$suppliername', supwarrantytype='$supwarrantytype', supcomplaintno='$supcomplaintno', supcomplaintremarks='$supcomplaintremarks', supcourierdate='$supcourierdate', supcourierpaytype='$supcourierpaytype', supcouriercharges='$supcouriercharges', supcourierdetails='$supcourierdetails', supcourierimg='$supcourierimg', taxablevalue='$taxablevalue' where id='$id'");
		if($sqlise)
		{
			$tid=$id;
...
<head>
  ...
  <title><?=$_SESSION['companyname']?> - Jerobyte - OEM Sent Details</title>
...
<div class="d-sm-flex align-items-center justify-content-between mb-1">
            <h1 class="h4 mb-2 mt-2 text-gray-800">OEM Sent Details</h1>
		...
		<div class="row">
			...
			<div class="table-responsive">
                <table class="table table-bordered font-13" width="100%" cellspacing="0">
                  ...
                  <tbody>
				  <?php 
		$sqlselect = "SELECT id,sourceid,callon,calltid,acknowlodge,callhandlingid,callhandlingname,coordinatorid,coordinatorname,engineertype,engineersid,engineersname,reportingengineerid,engineerid,engineername,compstatus,businesstype,servicetype,customernature,callnature,serial,diagnosisby,diagnosisengineername,diagnosiscoordinatorname,diagnosisremarks,diagnosismaterial,reportedproblem,problemobserved,actiontaken,narration,detailsid,otherremarks,dcno,suppliername,dcdate,supwarrantytype,supcomplaintno,supcomplaintremarks,supapprovalstatus,supestimatedcost,supestdelivery,supcompstatus,changeon,supcourierdate,supcourierpaytype,supcouriercharges,supcourierdetails,supcourierimg,taxablevalue From jrccalls where id='$id' order by id desc";
...
<div class="col-lg-3">
  <div class="form-group">
	...
	<hr>
	<h5 class="mb-3 text-primary">Send To OEM</h5>
	<div class="row">
<div class="col-lg-3">
  <div class="form-group">
    <label for="supcourierdate">Courier Date</label>
    <input type="date" class="form-control" id="supcourierdate" name="supcourierdate"  value="<?=$rowselect['supcourierdate']?>">
  </div>
</div>
<div class="col-lg-3">
  <div class="form-group">
    <label for="supcourierpaytype">Courier Pay Type</label>
	<select class="form-control" id="supcourierpaytype" name="supcourierpaytype">
	<option value="">Select</option>	
...
<div class="col-lg-3">
  <div class="form-group">
    <label for="supcouriercharges">Courier Charges</label>
    <input type="number" min="0" step="0.01" class="form-control" id="supcouriercharges" name="supcouriercharges"  value="<?=$rowselect['supcouriercharges']?>">
  </div>
</div>
<div class="col-lg-3">
  <div class="form-group">
    <label for="supcourierdetails">Courier Remarks</label>
...
<br />
  ...
  <div class="form-group">
    ...
    <input type="number" min="0" step="0.01" class="form-control" id="taxablevalue" name="taxablevalue"  value="<?=$rowselect['taxablevalue']?>">
  </div>
</div>
  </div>
