...
for ($i = 0; $i < count($data); $i++) {
	...
	for ($j = 0; $j < 39; $j++) {
		...
		if($j==13)
		{
			$arraddress1[]=mysqli_real_escape_string($connection,trim($data[$i][$j]));;
			$address1=mysqli_real_escape_string($connection,trim($data[$i][$j]));;
		}
		if($j==14)
		{
...
$warrantydate = date('Y-m-d', strtotime("+$off months", strtotime($overdate)));
	if($i>0)
	{
		...
		{
			$se=0;
		 if($_SESSION['companysiteurl']=='jrc')
		 {
