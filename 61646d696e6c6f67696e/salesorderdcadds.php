...
if(isset($_GET['dc']))
	 {
		 $dispatchdocno = mysqli_real_escape_string($connection, $_GET['dispatchdocno']);
		 $disthrough = mysqli_real_escape_string($connection, $_GET['disthrough']);
		 
	    mysqli_query($connection, "update jrcsrno set dnno = LPAD(dnno + 1, 4, '0')");
		...
		$query = "UPDATE jrctally SET dcno='DC / ".$currentMonthYear."/" . $row11['dnno'] . "' , dcdate='".$date."', disthrough='".$disthrough."', dispatchdocno='".$dispatchdocno."' WHERE sono='" . $_GET['id'] . "'";
		$result = mysqli_query($connection, $query);
		
	 
	 $sqlselect = "SELECT * From jrctally WHERE sono='".$_GET['id']."'";
     ...
     $rowCountselect = mysqli_num_rows($queryselect);   
			...
			if($rowCountselect > 0) 
		{
			...
			$invoicedate=mysqli_real_escape_string($connection1, $rowdc['invoicedate']);
		...
		{	
	     ...
	     $sqlup = "INSERT INTO jrcxl(createdon, createdby, encstatus, file_name, upload_time, invoiceno, invoicedate, tenderno, pono, podate, dcno, dcdate, installedon, installedby, maincategory, subcategory, consigneename, department, address1, address2, area, district, pincode, contact, phone, mobile, email, gstno, stockmaincategory, stocksubcategory, stockitem, invoicedqty, rate, gstper, overallwarranty, typeofproduct, componenttype, componentname, make, capacity, warranty, qty, serialnumber, sono, sodate, buyername, buyeraddress1, buyeraddress2, buyerarea, buyerdistrict, buyerstate, buyerpincode, buyercontact, buyerphone, buyermobile, buyeremail, buyergstno, buyergsttype, invoiceamt) VALUES ('$createdon', '$createdby', '0', 'From Sales Order', '$dateandtime', '$dcno', '$dcdate', '$tender', '$pono', '$podate', '$dcno', '$dcdate', '', '', '$conmaincategory', '$consubcategory', '$consigneename', '$condepartment', '$conaddress1', '$conaddress2', '$conaddress3', '$condistrict', '$conpincode', '$concontact', '$conphone', '$conmobile', '$conemail', '$congstno', '$promaincategory', '$prosubcategory', '$conproduct', '$invoiceqty', '$conunit', '$gstper', '$conwarranty', '$conprogroup', '$componenttype', '$componentname', '$conmake', '$concapacity', '$conwarranty', '$conqty', '$conserialno', '$sono', '$sodate', '$buyername', '$buyeraddress1', '$buyeraddress2', 'buyeraddress3', '$buyerdistrict', '$buyerstate', '$buyerpincode', '$buyercontact', '$buyerphone', '$buyermobile', '$buyermail', '$bgst', '$rtype', '$contotal')";
				...
				else
				{
				 ...
				 $sqlcon = "SELECT id From jrcconsignee WHERE id = '{$consigneeid}'";
					...
					else
					{
						...
						$sqlup1pro = "update jrcxl set productid='{$productid}',warrantycycle='{$conwarranty}' WHERE stockmaincategory = '{$promaincategory}' and stocksubcategory = '{$prosubcategory}' and stockitem = '{$conproduct}' and componenttype = '{$componenttype}' and componentname = '{$componentname}' and typeofproduct = '{$conprogroup}' and make = '{$conmake}' and capacity = '{$concapacity}'";
						$queryup1pro = mysqli_query($connection, $sqlup1pro);
 ...
 $sqlselect = "SELECT id, consigneeid, productid, serialnumber, departments, qty From jrcxl where invoiceno='" . $_GET['id'] . "' order by id asc";
$queryselect = mysqli_query($connection, $sqlselect);
$rowCountselect = mysqli_num_rows($queryselect);
if(!$queryselect){
    die("SQL query failed: " . mysqli_error($connection));
...
if($rowCountselect > 0) 
{
	...
	mysqli_query($connection, "INSERT INTO jrchistory (user, ipaddress, times, remarks, tid, tablename) VALUES ('{$useremail}', '{$ip}', '{$times}', 'Uploaded from Sales Order', '', 'jrcxl')");
	 header("Location:exporttally.php?remarks=DC Generated Successfully"); 	
		}
	 }
	 else
	 {
	 header("Location:exporttally.php?error=DC Not Generated"); 	
	 }
	 }
	 }
?>